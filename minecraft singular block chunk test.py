...
def update():
    ...
    while coordinateIterator != "end":
        ...
        if (down == 0 or down == None) and current == 1:
            verts.extend(generateVertBOTTOM(x, y, z))
            uvs.extend(generateUv())
            connections.extend(generateConnection(loopCounter))
            loopCounter += 1

        if (up == 0 or up == None) and current == 1:
            verts.extend(generateVertTOP(x, y, z))
            uvs.extend(generateUv())
            connections.extend(generateConnection(loopCounter))
            loopCounter += 1

        if (forward == 0 or forward == None) and current == 1:
            verts.extend(generateVertFRONT(x, y, z))
            uvs.extend(generateUv())
            connections.extend(generateConnection(loopCounter))
            loopCounter += 1

        if (backward == 0 or backward == None) and current == 1:
            verts.extend(generateVertBACK(x, y, z))
            uvs.extend(generateUv())
            connections.extend(generateConnection(loopCounter))
            loopCounter += 1

        if (right == 0 or right == None) and current == 1:
            verts.extend(generateVertRIGHT(x, y, z))
            uvs.extend(generateUv())
            connections.extend(generateConnection(loopCounter))
            loopCounter += 1

        if (left == 0 or left == None) and current == 1:
            verts.extend(generateVertLEFT(x, y, z))
            uvs.extend(generateUv())
            connections.extend(generateConnection(loopCounter))
            loopCounter += 1
...
def input(key):
    ...
    if key == "r":
        blocks.update({(test, 0, 0): 0})
        test += 1

    if key == "b":
        test -= 1
        blocks.update({(test,0,0):1})
